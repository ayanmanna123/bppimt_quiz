    ...
    return (
        ...
        >
            <div className="text-center space-y-4">
                <Suspense fallback={<h1 className="text-4xl font-bold text-slate-900 dark:text-white leading-tight">Welcome Messages</h1>}>
                    <SplitText
                ...
                {/* Social Icons Row */}
                <div className="flex justify-center gap-5 py-2 border-b border-slate-200/60 dark:border-indigo-500/20 max-w-sm mx-auto">
                ...
                {messages.map((msg, index) => (
                    <section key={index} className="space-y-6">
                        <div className="relative inline-block">
                            <h2 className="text-2xl md:text-4xl font-serif text-slate-800 dark:text-indigo-100 border-b-2 border-[#E11D48] dark:border-purple-500 pb-1">
                                {msg.title}
                            </h2>
                        </div>

                        <div className="space-y-4">
                            <h3 className="text-xl font-bold text-[#1A1A4E] dark:text-indigo-400">
                                {msg.name} <span className="text-slate-400 dark:text-indigo-200/40 font-light mx-1">|</span> {msg.role}
                            </h3>

                            <div className="flex flex-col md:flex-row gap-8 items-start">
                                <motion.div
                                    whileHover={{ scale: 1.01 }}
                                    className="w-full md:w-[320px] shrink-0 border-2 border-slate-900 dark:border-indigo-500/30 rounded shadow-sm overflow-hidden shadow-[0_0_20px_rgba(99,102,241,0.1)]"
                                >
                                    <img
                                        src={msg.image}
                                        alt={msg.name}
                                ...
                                <div className="flex-1 space-y-4 text-slate-700 dark:text-indigo-100/80 leading-relaxed text-lg font-medium">
                                    {msg.content.map((p, i) => (
                                        <p key={i}>{p}</p>
                                    ))}
                                </div>
