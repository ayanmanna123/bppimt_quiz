    ...
    return (
        <div className="min-h-screen bg-slate-50 pt-10 pb-20 px-4 md:px-8">
            ...
            <div className="fixed inset-0 pointer-events-none overflow-hidden -z-10">
                <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-blue-100/50 rounded-full blur-3xl opacity-30 -translate-y-1/2 translate-x-1/4"></div>
                <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-purple-100/50 rounded-full blur-3xl opacity-30 translate-y-1/2 -translate-x-1/4"></div>
            </div>
            ...
            <div className="max-w-5xl mx-auto">
                ...
                <motion.div
                    ...
                    className="bg-white/70 backdrop-blur-xl border border-slate-200/60 p-6 md:p-8 rounded-3xl shadow-2xl mb-12"
                >
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        {/* Year Selector */}
                        <div className="space-y-2">
                            <label className="text-sm font-bold text-slate-700 flex items-center gap-2 pl-1">
                                <Calendar className="w-4 h-4 text-blue-600" />
                                Select Year
                            </label>
                            <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                    <Button variant="outline" className="w-full justify-between h-14 rounded-2xl border-2 hover:border-blue-400 transition-all font-medium text-slate-700">
                                        {selectedYear || "Choose Academic Year"}
                                        <ChevronDown className="w-4 h-4 opacity-50" />
                                    </Button>
                                </DropdownMenuTrigger>
                                <DropdownMenuContent className="w-[calc(100vw-3rem)] md:w-64 rounded-2xl p-2">
                                    {years.map(y => (
                                        <DropdownMenuItem
                                            key={y}
                                            onClick={() => setSelectedYear(y)}
                                            className="rounded-xl cursor-pointer p-3 focus:bg-blue-50 focus:text-blue-600"
                                        >
                                            {y}
                                        </DropdownMenuItem>
                                    ))}
                        ...
                        <div className="space-y-2">
                            ...
                            <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                    <Button variant="outline" className="w-full justify-between h-14 rounded-2xl border-2 hover:border-indigo-400 transition-all font-medium text-slate-700">
                                        {streams.find(s => s.value === selectedStream)?.label || "Choose Your Stream"}
                                        <ChevronDown className="w-4 h-4 opacity-50" />
                                    </Button>
                                </DropdownMenuTrigger>
                                <DropdownMenuContent className="w-[calc(100vw-3rem)] md:w-72 rounded-2xl p-2">
                                    {streams.map(s => (
                                        <DropdownMenuItem
                                            key={s.value}
                                            onClick={() => setSelectedStream(s.value)}
                                            className="rounded-xl cursor-pointer p-3 focus:bg-indigo-50 focus:text-indigo-600"
                                        >
                                            {s.label}
                                        </DropdownMenuItem>
                                    ))}
                        ...
                        <div className="space-y-2">
                            ...
                            <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                    <Button variant="outline" className="w-full justify-between h-14 rounded-2xl border-2 hover:border-purple-400 transition-all font-medium text-slate-700">
                                ...
                                <DropdownMenuContent className="w-[calc(100vw-3rem)] md:w-64 rounded-2xl p-2">
                                    {semesters.map(sem => (
                                        <DropdownMenuItem
                                            ...
                                            className="rounded-xl cursor-pointer p-3 focus:bg-purple-50 focus:text-purple-600"
                                        >
                                            {sem} SEMESTER
                                        </DropdownMenuItem>
                                    ))}
                    ...
                    {showResults && (
                        ...
                        >
                            <div className="flex items-center justify-between px-2">
                                <div className="flex items-center gap-2 text-slate-800 font-bold text-xl uppercase tracking-wider">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {currentSubjects.map((subject, idx) => (
                                    <motion.div
                                        ...
                                        className="group bg-white border border-slate-200 p-5 rounded-2xl flex items-center justify-between shadow-sm hover:shadow-md hover:border-blue-200 transition-all cursor-default"
                                    >
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs font-semibold text-slate-400 group-hover:text-slate-500 uppercase tracking-tighter">
                                        ...
                                        <a
                                            ...
                                            className="p-3 rounded-xl bg-slate-50 text-slate-600 hover:bg-blue-600 hover:text-white transition-all transform active:scale-95 shadow-inner"
                            ...
                            >
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        <h4 className="font-bold text-amber-900 mb-1">Developer Note</h4>
                                        <p className="text-sm text-amber-800 leading-relaxed">
                                            To make these downloads work, please ensure the PDF files are uploaded to your <code className="bg-amber-100 px-1 rounded font-mono">public/PYQ/</code> folder following the structure:
                                            <br />
                                            <span className="font-mono text-xs mt-2 block bg-white/50 p-2 rounded">
