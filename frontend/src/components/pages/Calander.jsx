  ...
  const renderCalendarDays = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div
          key={day}
          className={`h-32 p-1 border border-gray-200 bg-white overflow-hidden ${
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-600">
                  Duration: {quiz.time} minutes
                </span>
              </div>
              <div className="flex items-center gap-2">
                <User className="w-4 h-4 text-purple-500" />
                <span className="text-sm text-gray-600">
                  Created by: {quiz.createdBy.fullname}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <BookOpen className="w-4 h-4 text-orange-500" />
                <span className="text-sm text-gray-600">
                  Questions: {quiz.totalQuestions} | Marks: {quiz.marks} each
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-7 gap-0 mb-2">
            {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
              <div
                key={day}
                className="h-10 flex items-center justify-center font-medium text-gray-600 bg-gray-50"
              >
                {day}
              </div>
            ))}
